...
## 5a) Natywne powiadomienia Android (FCM)

Cloud obsługuje natywne push dla aplikacji `WM Sprinkler`:
- rejestr tokenu: `POST /api/mobile/push/register`
- usunięcie tokenu: `POST /api/mobile/push/unregister`
- test push: `POST /api/mobile/push/test`

W `.env` ustaw:

```env
MOBILE_PUSH_ENABLED=true
FCM_ANDROID_CHANNEL_ID=wms_alerts
FCM_DEFAULT_TTL_SEC=1800
MOBILE_PUSH_MIN_EVENT_GAP_MS=8000
MOBILE_PUSH_MAX_TOKENS_PER_USER=8

# jedno źródło klucza usługi:
FCM_SERVICE_ACCOUNT_PATH=/var/www/wmsprinkler/secrets/firebase-service-account.json
# lub FCM_SERVICE_ACCOUNT_JSON=...
# lub FCM_SERVICE_ACCOUNT_JSON_B64=...
```

Doinstaluj zależność i restart PM2:

```bash
cd /var/www/wmsprinkler/cloud
npm install
pm2 restart wms-cloud --update-env
```
