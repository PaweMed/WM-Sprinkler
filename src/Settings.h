...
public:  // GETTERY (używane przez Config/MQTT/Weather/itp.)
  ...
  void load() {
    ...
    mqttServer    = prefs.getString("mqttServer", WMS_DEFAULT_MQTT_SERVER);
    mqttUser      = prefs.getString("mqttUser", WMS_DEFAULT_MQTT_USER);
    mqttPass      = prefs.getString("mqttPass", WMS_DEFAULT_MQTT_PASS);
    mqttClientId  = prefs.getString("mqttClientId", "");
    mqttPort      = prefs.getInt("mqttPort", WMS_DEFAULT_MQTT_PORT);
    enableMqtt    = prefs.getBool("enableMqtt", true);
    mqttTopicBase = prefs.getString("mqttTopicBase", "");

    autoMode  = prefs.getBool("autoMode", true);
    zoneCount = normalizeZoneCount(prefs.getInt("zoneCount", 8));

    timezone  = normalizeTimezoneValue(prefs.getString("timezone", "Europe/Warsaw"));

    // NVS key names in Preferences should stay short (<=15 chars on ESP32).
    enableWeatherApi          = prefs.getBool("enWeatherApi", true);
    weatherUpdateIntervalMin  = prefs.getInt("weatherUpdMin", 60);
    irrigationTempSkipC = prefs.getFloat("irTmpSkip", 5.0f);
    ...
    irrigationTempLowMaxC = prefs.getFloat("irTmpLowMx", 18.0f);
    irrigationTempMidMaxC = prefs.getFloat("irTmpMidMx", 25.0f);
    irrigationTempHighMaxC = prefs.getFloat("irTmpHiMx", 30.0f);
    irrigationTempFactorLow = prefs.getFloat("irTmpLowF", 0.5f);
    irrigationTempFactorMid = prefs.getFloat("irTmpMidF", 1.0f);
    irrigationTempFactorHigh = prefs.getFloat("irTmpHiF", 1.3f);
    irrigationTempFactorVeryHigh = prefs.getFloat("irTmpVHiF", 1.5f);
    irrigationRainSkipMm = prefs.getFloat("irRainSkp", 6.0f);
    irrigationRainHighMinMm = prefs.getFloat("irRainHiMn", 5.0f);
    irrigationRainMidMinMm = prefs.getFloat("irRainMdMn", 2.0f);
    irrigationRainFactorHigh = prefs.getFloat("irRainHiF", 0.3f);
    irrigationRainFactorMid = prefs.getFloat("irRainMdF", 0.6f);
    irrigationRainFactorLow = prefs.getFloat("irRainLoF", 1.0f);
    irrigationHumidityHighPercent = prefs.getFloat("irHumHi", 90.0f);
    irrigationHumidityFactorHigh = prefs.getFloat("irHumHiF", 0.75f);
    irrigationWindSkipKmh = prefs.getFloat("irWndSkp", 25.0f);
    irrigationWindFactor = prefs.getFloat("irWndFac", 1.0f);
    irrigationPercentMin = prefs.getInt("irPctMin", 0);
    irrigationPercentMax = prefs.getInt("irPctMax", 160);
