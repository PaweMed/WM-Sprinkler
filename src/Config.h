...
class Config {
  // Ta płytka ma LED statusową pod elementem D20, podłączoną do GPIO23.
  ...
  static constexpr unsigned long apModeDurationMs = 5UL * 60UL * 1000UL;
  ...
  void ensureWiFiEventHandler() {
    ...
    WiFi.onEvent([this](WiFiEvent_t, WiFiEventInfo_t info) {
      lastDisconnectReasonCode = (int)info.wifi_sta_disconnected.reason;
  ...
  bool apModeWindowExpired() {
    ...
    return (millis() - apModeStartedMs) >= apModeDurationMs;
  }
...
public:
  ...
  void connectWiFi(bool startupAttempt = false) {
    ...
    if (finalStatus == WL_CONNECTED) {
      ...
      Serial.println();
      failedWiFiAttempts++;
      // Po kilku nieudanych próbach przechodzimy do AP na 5 minut.
