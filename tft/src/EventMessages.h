...
inline String legacyFieldValue(const String& text, const char* key) {
...
inline String displayMessageText(const String& rawLine) {
  ...
  const bool strefa = text.startsWith("[STREFA]");
  const bool wmsc = text.startsWith("[WMSC]");
  const bool harmonogram = text.startsWith("[HARMONOGRAM]");
  const bool gniazdko = text.startsWith("[GNIAZDKO]");
  const bool ota = text.startsWith("[OTA]");

  if (text.indexOf(" | ") >= 0) {
    const String action = stripCategoryPrefix(text.substring(0, text.indexOf(" | ")));
    const String object = normalizeZoneObject(legacyFieldValue(text, "obiekt"));
    const String mins = legacyFieldValue(text, "czas");
    const String actual = legacyFieldValue(text, "wykonano");
    const String detail = legacyFieldValue(text, "szczegoly");
    const String reason = legacyFieldValue(text, "powod");
    const String target = legacyFieldValue(text, "target");
    const String version = legacyFieldValue(text, "wersja");
    const String result = legacyFieldValue(text, "wynik");

    if ((strefa || wmsc) && action == "start" && object.length()) return prefix + ensureSentence(object + " uruchomiono" + (mins.length() ? " na " + mins : ""));
    if ((strefa || wmsc) && action == "stop" && object.length()) return prefix + ensureSentence(object + " wyłączono");
    if ((strefa || wmsc) && action == "aktualizacja" && object.length()) return prefix + ensureSentence(object + " zaktualizowano" + (mins.length() ? " do " + mins : ""));
    if (harmonogram && action == "pominieto" && object.length()) return prefix + ensureSentence(object + " anulowano - " + simplifyReason(detail.length() ? detail : reason));
    if (harmonogram && (action == "przeliczono" || action == "start") && object.length()) {
      const String duration = actual.length() ? actual : mins;
      return prefix + ensureSentence(object + " uruchomiono" + (duration.length() ? " na " + duration : ""));
    }
    if (gniazdko && action.length()) {
      const String plug = legacyFieldValue(text, "obiekt");
      if (action == "start" || action.indexOf("włącz") >= 0) return prefix + ensureSentence(String(plug) + " włączono" + (mins.length() ? " na " + mins : ""));
      if (action == "stop" || action.indexOf("wyłącz") >= 0) return prefix + ensureSentence(String(plug) + " wyłączono");
    }
    if (ota) {
      String targetLabel = target;
      if (targetLabel == "firmware") targetLabel = "firmware";
      else if (targetLabel == "fs") targetLabel = "plików WWW";
      else if (!targetLabel.length()) targetLabel = "pakietu";

      if (action == "start") {
        String sentence = "Rozpoczęto aktualizację OTA " + targetLabel;
        if (version.length()) sentence += " do wersji " + version;
        return prefix + ensureSentence(sentence);
      }
      if (action == "sukces") {
        String sentence = "Aktualizacja OTA " + targetLabel + " zakończona";
        if (version.length()) sentence += " (" + version + ")";
        if (result.length()) sentence += " - " + result;
        return prefix + ensureSentence(sentence);
      }
      if (action == "blad" || action == "błąd") {
        return prefix + ensureSentence(String("Błąd OTA") + (reason.length() ? " - " + reason : ""));
      }
      if (action == "blad backupu" || action == "błąd backupu") {
        return prefix + ensureSentence(String("Błąd backupu OTA") + (reason.length() ? " - " + reason : ""));
      }
      if (action == "ostrzezenie" || action == "ostrzeżenie") {
        String sentence = "Ostrzeżenie OTA";
        if (reason.length()) sentence += " - " + reason;
        if (detail.length()) sentence += ": " + detail;
        return prefix + ensureSentence(sentence);
      }
    }
  }
