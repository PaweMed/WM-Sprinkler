...
function injectMainAppCloudOverlay(html) {
  ...
  const snippet = '  <script defer src="/cloud-overlay/weather-percent-overlay.js?v=20260315-5"></script>';
...
function buildUnifiedWeatherSnapshot(state) {
  ...
  const observedRainCandidates = [
    ...
    Array.isArray(state?.rainHistory) ? sumRainHistory(state.rainHistory, 24) : NaN,
  ].filter((v) => Number.isFinite(v));
  if (observedRainCandidates.length) {
    // Prefer the most conservative/realistic 24h rainfall across all live sources.
    const mm = Number(Math.max(...observedRainCandidates.map((v) => Math.max(0, v))).toFixed(1));
